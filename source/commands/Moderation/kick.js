...
module.exports = class extends Command {
  constructor(...args) {
    ...
    this.customizeResponse('userToKick',
      'You need to give me people to kick.'
    );
  }

  async run(msg, [userToKick, reason]) {
    if (userToKick.id === msg.author.id) throw 'Why do you want to kick yourself?';
    if (userToKick.id === this.client.user.id) throw 'Have I been a bad bot?';

    if (userToKick.roles.highest.position >= msg.userToKick.roles.highest.position) throw 'You can\'t kick that person.';
    if (!userToKick.kickable) throw 'I can\'t kick that person.';
