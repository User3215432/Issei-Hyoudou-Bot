const { Command, Duration } = require('Nasa');
...
module.exports = class extends Command {
  constructor(...args) {
    ...
    this.customizeResponse('userToMute',
      'You need to give someone to mute.'
    );
  }

  async run(msg, [duration, userToMute, reason]) {
    if (userToMute.id === msg.author.id) throw 'Why do you want to mute yourself?';
    if (userToMute.id === this.client.user.id) throw 'Have I been a bad bot?';
    if (userToMute.roles.highest.position >= msg.userToMute.roles.highest.position) throw 'You can\'t mute that person.';

    if (userToMute.roles.has(msg.guild.settings.roles.muted)) throw 'That person is already muted.';
