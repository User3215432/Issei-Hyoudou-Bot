...
module.exports = class extends Command {
  ...
  async run(message) {
    ...
    if (!music.queue.length) return message.send(`You need to add a song to the queue with the \`${message.guild.settings.prefix}add\` command, then use the \`${message.guild.settings.prefix}play\` command.`);
