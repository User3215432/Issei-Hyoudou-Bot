...
module.exports = class extends Command {
  ...
  async run(message, [...searchTerm]) {
    ...
    const wiki = await request(link)
      ...
      .catch(err => {
        if (err.statusCode === 403) throw message.send('Wikipedia is down, try again later.');
        if (err.statusCode === 404) throw message.send('I couldn\'t find that article on Wikipedia.');
        else {
